...
- Stock is tracked for `ON_HAND`. `PRE_ORDER` has no stock cap. `sku.isTester` (awarded via the decant promo, excluded from the catalog) is a per-SKU checkbox on `FULL_BOTTLE`/`PARTIAL` SKUs — a decant is never given away as a bonus tester unit, so it's hidden (preserved via a hidden input) on decant SKU forms rather than exposed as a product-level setting. Don't confuse this with `sku.provenance = 'TESTER'`, a separate field describing a non-decant physical bottle's origin (shown as a badge, unrelated to the tester-bonus pool).
