...
- Decant promo: when the discounted merchandise subtotal of decants reaches ₱2,000, the order gets free delivery AND one tester drawn from a brand the customer purchased (`pickTester` matches `brand` only; `upsertSku` copies that onto `sku.testerBrand`). If no compatible tester is in stock or the reserve races and loses, leave the result `PENDING` for admin fulfillment — never `SKIPPED`, never a random unrelated tester.
